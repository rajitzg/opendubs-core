...
class LidarMerger(Node):
    ...
    def resample_scan(self, scan_ranges, scan_msg, target_increment):
        ...
        original_angles = scan_msg.angle_min + \
            np.arange(ranges.size, dtype=np.float32) * scan_msg.angle_increment
    ...
    def transform_scan(self, ranges, scan_msgs):
        try:
            ...
            self.get_logger().warn(
                f'Could not transform {scan_msgs.header.frame_id} '
    ...
    def points_to_ranges(self, points, scan_msg):
        ...
        for point in points:
            ...
            idx = (
                int(round((angle - scan_msg.angle_min) / scan_msg.angle_increment))
                % num_readings
            )
    ...
    def merge_scans(self, scan1_msg, scan2_msg):
        scan1_stamp_ns = (
            scan1_msg.header.stamp.sec * 1_000_000_000 + scan1_msg.header.stamp.nanosec
        )
        scan2_stamp_ns = (
            scan2_msg.header.stamp.sec * 1_000_000_000 + scan2_msg.header.stamp.nanosec
        )
        ...
        resampled_ranges1 = self.resample_scan(
            scan1_msg.ranges,
            scan1_msg,
            merged_scan.angle_increment,
        )
        resampled_ranges2 = self.resample_scan(
            scan2_msg.ranges,
            scan2_msg,
            merged_scan.angle_increment,
        )
