...
public:
  ...
  double calculate(double target, double feedback, double dt)
  {
    ...
    state_.integral =
      std::clamp(state_.integral, -config_.integrity_limit, config_.integrity_limit);
    ...
    double output =
      (config_.k_p * error) + (config_.k_i * state_.integral) + (config_.k_d * derivative);
