...
void LLControlNode::computePID()
{
    ...
    // This would use current_odom_ and rc_fwd_/lat_/yaw_ (as velocity targets)
    // to compute new corrected output PWMs.
    
    // For now in VELOCITY mode, we might just pass through or do simple open-loop scaling if needed,
    // but the structure is here to add error computation:
    // double error_x = target_vel_x - current_odom_.twist.twist.linear.x;
    // ...
}
...
void LLControlNode::controlLoop()
{
  ...
  // Safety check: if no RC for > 1s, failsafe to release overrides (0) or stop (1500)?
  // MAVROS might handle RC loss too, but let's be safe. 
  // If we haven't heard from RC, we shouldn't be driving.
  if ((this->now() - last_rc_msg_time_).seconds() > 1.0) {
      ...
      // Actually, safest is to do nothing and let ArduPilot failsafe kick in,
      // which happens when RC_CHANNELS_OVERRIDE stops updating for RC_OVERRIDE_TIME.
      // So checking timestamps here for logic is good, but maybe we just stop publishing or publish "Release" (0).
      return; 
  }
  ...
  if (current_mode_ == ControlMode::MANUAL) {
      ...
      computePID(); // Updates internal state, maybe modifies output variables
      // For now, pass through as placeholder until PID implemented
      out_fwd = rc_fwd_;
      out_lat = rc_lat_;
      out_yaw = rc_yaw_;
  } else if (current_mode_ == ControlMode::AUTO) {
      // Auto logic from external planner signals or internal waypoints
      computePID();
      out_fwd = 1500; // Stop for now as safety default
      out_lat = 1500;
      out_yaw = 1500;
  }
