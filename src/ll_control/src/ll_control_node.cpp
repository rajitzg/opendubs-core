...
: Node("ll_control_node")
{
  ...
  // Safety
  cmd_vel_timeout_threshold_ =
    this->declare_parameter("cmd_vel_timeout_threshold", cmd_vel_timeout_threshold_);
  ...
  cmd_vel_sub_ = this->create_subscription<geometry_msgs::msg::Twist>(
    "/teleop/cmd_vel", 10, std::bind(&LLControlNode::cmdVelCallback, this, std::placeholders::_1));

  mode_sub_ = this->create_subscription<std_msgs::msg::Int8>(
    "/teleop/control_mode",
    rclcpp::QoS(10)
    .reliable()
    .transient_local(),    // transient_local so we get the last mode on startup
  ...
  // Publisher
  rc_override_pub_ =
    this->create_publisher<mavros_msgs::msg::OverrideRCIn>("/mavros/rc/override", 10);
...
LLControlNode::~LLControlNode()
{
  RCLCPP_WARN(
    this->get_logger(),
    "Shutting down LL Control Node. Switching to HOLD and resetting RC Overrides to Neutral "
    "(1500).");
...
void LLControlNode::controlLoop()
{
  ...
  if (current_mode_ != ControlMode::MANUAL) {
    if (imu_timeout) {
      RCLCPP_ERROR_THROTTLE(
        this->get_logger(), *this->get_clock(), 2000, "CRITICAL: IMU Lost! Switching to HOLD.");
      setArduPilotMode(mode_hold_name_);
      ...
      if (now_sec - last_freq_fix_time_.seconds() > imu_fix_period_) {
        last_freq_fix_time_ = this->now();
        RCLCPP_WARN(
          this->get_logger(), "IMU rate low: %.1f Hz. Attempting to fix rate.", current_imu_freq_);
        std::thread([this]() {
            std::string cmd = "ros2 run mavros mav sys rate --all " +
            std::to_string(static_cast<int>(this->imu_target_freq_));
  ...
  switch (current_mode_) {
    ...
    case ControlMode::AUTO:
      double fwd_effort = pid_x_.calculate(current_cmd_vel_.linear.x, 0.0 /*No Odom yet*/, dt);
      double lat_effort = pid_y_.calculate(current_cmd_vel_.linear.y, 0.0 /*No Odom yet*/, dt);
      double yaw_effort = pid_yaw_.calculate(current_cmd_vel_.angular.z, current_yaw_rate_, dt);
...
void LLControlNode::setArduPilotMode(const std::string & mode)
{
  ...
  if (debug_mode) {
    RCLCPP_INFO_SKIPFIRST_THROTTLE(
      this->get_logger(), *this->get_clock(), 2000,
      "Debug mode enabled: skipping SetMode request to '%s'", mode.c_str());
    return;
  }

  if (!set_mode_client_->wait_for_service(std::chrono::milliseconds(100))) {
    RCLCPP_ERROR_THROTTLE(
      this->get_logger(), *this->get_clock(), 2000, "SetMode service not available");
    return;
  }
...
rcl_interfaces::msg::SetParametersResult LLControlNode::parametersCallback(
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  for (const auto & param : parameters) {
    ...
    // PID X
    if (name == "pid_vel_x.k_p") {
      cfg_x.k_p = param.as_double();
      update_x = true;
    } else if (name == "pid_vel_x.k_i") {
      cfg_x.k_i = param.as_double();
      update_x = true;
    } else if (name == "pid_vel_x.k_d") {
      cfg_x.k_d = param.as_double();
      update_x = true;
    } else if (name == "pid_vel_x.k_ff") {
      cfg_x.k_ff = param.as_double();
      update_x = true;
    } else if (name == "pid_vel_x.max_output") {
      cfg_x.max_output = param.as_double();
      update_x = true;
    }
    // PID Y
    else if (name == "pid_vel_y.k_p") {
      cfg_y.k_p = param.as_double();
      update_y = true;
    } else if (name == "pid_vel_y.k_i") {
      cfg_y.k_i = param.as_double();
      update_y = true;
    } else if (name == "pid_vel_y.k_d") {
      cfg_y.k_d = param.as_double();
      update_y = true;
    } else if (name == "pid_vel_y.k_ff") {
      cfg_y.k_ff = param.as_double();
      update_y = true;
    } else if (name == "pid_vel_y.max_output") {
      cfg_y.max_output = param.as_double();
      update_y = true;
    }
    // PID Yaw
    else if (name == "pid_yaw_rate.k_p") {
      cfg_yaw.k_p = param.as_double();
      update_yaw = true;
    } else if (name == "pid_yaw_rate.k_i") {
      cfg_yaw.k_i = param.as_double();
      update_yaw = true;
    } else if (name == "pid_yaw_rate.k_d") {
      cfg_yaw.k_d = param.as_double();
      update_yaw = true;
    } else if (name == "pid_yaw_rate.k_ff") {
      cfg_yaw.k_ff = param.as_double();
      update_yaw = true;
    } else if (name == "pid_yaw_rate.max_output") {
      cfg_yaw.max_output = param.as_double();
      update_yaw = true;
    }
    // Safety
    else if (name == "cmd_vel_timeout_threshold") {
      cmd_vel_timeout_threshold_ = param.as_double();
    } else if (name == "imu_timeout_threshold") {
      imu_timeout_threshold_ = param.as_double();
    } else if (name == "imu_fix_period") {
      imu_fix_period_ = param.as_double();
    } else if (name == "imu_target_freq") {
      imu_target_freq_ = param.as_double();
    }
  }
  ...
  if (update_x) {
    pid_x_.configure(cfg_x);
    RCLCPP_INFO(
      this->get_logger(), "Updated PID X: P=%.2f I=%.2f D=%.2f FF=%.2f", cfg_x.k_p, cfg_x.k_i,
      cfg_x.k_d, cfg_x.k_ff);
  }
  if (update_y) {
    pid_y_.configure(cfg_y);
    RCLCPP_INFO(
      this->get_logger(), "Updated PID Y: P=%.2f I=%.2f D=%.2f FF=%.2f", cfg_y.k_p, cfg_y.k_i,
      cfg_y.k_d, cfg_y.k_ff);
  }
  if (update_yaw) {
    pid_yaw_.configure(cfg_yaw);
    RCLCPP_INFO(
      this->get_logger(), "Updated PID Yaw: P=%.2f I=%.2f D=%.2f FF=%.2f", cfg_yaw.k_p, cfg_yaw.k_i,
      cfg_yaw.k_d, cfg_yaw.k_ff);
  }
